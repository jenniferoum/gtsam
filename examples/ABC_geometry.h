 ...
 */
#ifndef ABC_GEOMETRY_H
#define ABC_GEOMETRY_H
/**
 * @file ABC.h
 * @brief Core components for Attitude-Bias-Calibration systems
 *
...
struct ABCGeometry {
  ...
  static Vector lift(const MType& x, const Input& u);
  static GType groupExp(const Vector& v);
...
template <size_t N>
Vector abc_eqf_lib::ABCGeometry<N>::lift(const State<N>& xi, const Input& u) {
  Vector L = Vector::Zero(6 + 3 * N);
...
template <size_t N>
abc_eqf_lib::G<N> abc_eqf_lib::ABCGeometry<N>::groupExp(const Vector& v) {
  return G<N>::exp(v);
}

template <size_t N>
abc_eqf_lib::G<N> abc_eqf_lib::ABCGeometry<N>::groupCompose(const G<N>& g1,
                                                            const G<N>& g2) {
  return g1 * g2;
}

template <size_t N>
Matrix abc_eqf_lib::ABCGeometry<N>::inputMatrix(G<N> X_hat) {
  Matrix B1 = blockDiag(X_hat.A.matrix(), X_hat.A.matrix());
  Matrix B2(3 * N, 3 * N);
 ...
 */
template <size_t N>
Matrix measurementMatrixC(const Unit3& d, int idx) {
  Matrix Cc = Matrix::Zero(3, 3 * N);
 ...
 */
template <size_t N>
Matrix outputMatrixDt(int idx, abc_eqf_lib::G<N> X_hat) {
  // If the measurement is related to a sensor that has a calibration state
  if (idx >= 0) {
    if (idx >= static_cast<int>(N)) {
      throw std::out_of_range("Calibration index out of range");
